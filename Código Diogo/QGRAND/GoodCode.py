...
class GoodCode:
    ...
    def _set_syndrome_information(self):
        ...
        # Get unique syndromes, while preserving the order
        # See https://stackoverflow.com/a/12926989
        self.unique_syndrome_matrix = self.syndrome_matrix[np.sort(np.unique(self.syndrome_matrix, axis=0, return_index=True)[1])]
    ...
    def _get_degeneracy_information(syndrome_matrix,
        ...
        # List of lists of errors with the same syndrome.
        repeated_idx = []
        single_idx = []
        ...
        #print(single_idx, repeated_idx)
        # List of pairs of errors with the same syndrome, with the first error
        # considered the coset leader.
        ...
        for idx in repeated_idx:
            for ind in idx[1:]:
                ...
                s += 1
        # List of product of these error pairs
        ...
        # Swapping Z and X halves of errors, for comparison with parity check 
        # matrix.
    ...
    def _get_boolean_solution(m, error):
        ...
        error : 1D array, 2D array
            2n sized error, or (P x 2n) product error matrix.
